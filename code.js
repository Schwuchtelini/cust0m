...
'        $(".comment-foot").each(function(index, elem) { if(true || $(elem).find(".cust0m_ups_downs_comment").length == 0) $(elem).find(".score").after("<div class=\'cust0m_ups_downs_comment\'>" + $(elem).find(".score").attr("title") + "<div>"); if($(elem).find(".user").text() == op) { $(elem).addClass("custom_op"); if(cust0m.op_top && (parseInt($(elem).attr("tiefe")) == 0)) { if($(elem).parent().next().hasClass("comment-box")) $(elem).parent().next().prependTo(".comments>.comment-box"); $(elem).parent().prependTo(".comments>.comment-box"); }} });\r\n' +
