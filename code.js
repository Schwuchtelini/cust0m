...
'           data = data2;\r\n' +
'           if(data.items.length < 40 && position == p.Stream.POSITION.APPEND && !stream.reached.end) { console.log("Lade end"); stream._loadCust0m({older: stream._oldestId}, callback, data); }\r\n' +
'           else if(data.items.length < 40 && !stream.reached.start) { console.log("Lade anfang"); stream._loadCust0m({newer: stream._newestId}, callback, data); }\r\n' +
'           else \r\n' +
'           {\r\n' +
