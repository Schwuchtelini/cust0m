...
'        $(".comment-foot").each(function(index, elem) { if($(elem.score).find(".cust0m_ups_downs_comment").size() == 0) $(elem.score).after("<div class=\'cust0m_ups_downs_comment\'>" + $(elem.score).attr("title") + "<div>"); if($(elem).find(".user").text() == op) { $(elem).addClass("custom_op"); if(cust0m.op_top && (parseInt($(elem).attr("tiefe")) == 0)) { if($(elem).parent().next().hasClass("comment-box")) $(elem).parent().next().prependTo(".comments>.comment-box"); $(elem).parent().prependTo(".comments>.comment-box"); }} });\r\n' +
